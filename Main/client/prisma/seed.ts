...
async function main() {
    ...
    // }
        ...
        const walter = await prisma.admin.findUnique({
    where: { username: "WalterBlack" },
    });

    if (!walter) {
    const hashedPassword = await bcrypt.hash("password123", 10);
      
    const existingLower = await prisma.admin.findUnique({
        where: { username: "walterblack" },
    });
    ...
    if (existingLower) {
        ...
        console.log("Created Owner: WalterBlack");
    }
    } else {
    await prisma.admin.update({
        where: { id: walter.id },
